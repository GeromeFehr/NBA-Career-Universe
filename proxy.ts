...
export async function proxy(request:NextRequest){
 ...
 if(request.nextUrl.pathname.startsWith("/api/")&&!["GET","HEAD","OPTIONS"].includes(request.method)){
  ...
  if(!valid||site==="cross-site")return NextResponse.json({error:lang==="en"?"Request origin rejected.":"Anfrageherkunft nicht zulässig."},{status:403});
  if(Number(request.headers.get("content-length")||0)>9_500_000)return NextResponse.json({error:lang==="en"?"Upload too large.":"Upload zu groß."},{status:413});
 }
 ...
 const supabase=createServerClient(url,key,{cookies:{getAll:()=>request.cookies.getAll(),setAll:values=>{values.forEach(({name,value})=>request.cookies.set(name,value));h.set("cookie",request.cookies.toString());response=NextResponse.next({request:{headers:h}});values.forEach(({name,value,options})=>response.cookies.set(name,value,options));}}});
