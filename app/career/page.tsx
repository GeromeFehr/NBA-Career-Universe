...
export default async function Page(){
 ...
 return <>
 <section className="hero"><span className="eyebrow">{universe.name} · CAREER FILE</span><h1>{career.player_name}</h1><p>{career.position} · OVR {career.overall} · Draft {career.draft_year||"—"} #{career.draft_pick||"—"}</p><div className="heroBar">{(stints||[]).map((st:any)=><div key={st.id}><span>{st.start_date} – {st.end_date||today}</span><strong><TeamBadge team={st.teams} small/> {st.teams?.abbreviation}</strong></div>)}</div></section>
 <div className="sectionHead"><h2>{t(lang,"careerValues")}</h2></div><div className="statGrid"><StatCard label="Games" value={x.games}/><StatCard label="PPG" value={x.ppg.toFixed(1)}/><StatCard label="RPG" value={x.rpg.toFixed(1)}/><StatCard label="APG" value={x.apg.toFixed(1)}/><StatCard label="SPG" value={x.spg.toFixed(1)}/><StatCard label="BPG" value={x.bpg.toFixed(1)}/><StatCard label="High PTS" value={x.careerHighPoints}/><StatCard label="High BLK" value={x.careerHighBlocks}/></div>
 <div className="dashboardGrid"><section><div className="sectionHead"><h2>{t(lang,"timeline")}</h2></div><div className="timeline">{(events||[]).map((e:any)=><div className="timelineItem" key={e.id}><small>{e.event_date} · {e.event_type}</small><h3>{e.title||e.event_type}</h3><p>{e.description}</p></div>)}</div></section>
 <aside><div className="sectionHead"><h2>{t(lang,"injuries")}</h2></div>{(injuries||[]).map((i:any)=><div className="card" key={i.id}><b>{i.injury}</b><p>{i.start_date} · {i.severity}</p><span className="pill">{i.status}</span></div>)}<div className="sectionHead"><h2>{t(lang,"milestones")}</h2></div>{(milestones||[]).map((m:any)=><div className="card" key={m.id}><b>{m.title}</b><p className="muted">{m.description}</p></div>)}</aside></div>
 <div className="sectionHead"><h2>{t(lang,"gameLog")}</h2></div><div className="tableWrap"><table><thead><tr><th>Team</th><th>PTS</th><th>REB</th><th>AST</th><th>STL</th><th>BLK</th><th>TO</th><th>FG</th><th>Status</th></tr></thead><tbody>{(s||[]).map((g:any)=><tr key={g.id}><td>{g.team?.abbreviation}</td><td>{g.points}</td><td>{g.rebounds}</td><td>{g.assists}</td><td>{g.steals}</td><td>{g.blocks}</td><td>{g.turnovers}</td><td>{g.fgm}/{g.fga}</td><td>{g.appearance_status}</td></tr>)}</tbody></table></div></>;
}
