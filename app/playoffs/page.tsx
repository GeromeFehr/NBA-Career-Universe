...
export default async function Page(){
  ...
  for(const g of games){
    ...
    const key=String(g.stage)+"|"+String(opponent?.abbreviation||"TBA");
    if(!groups.has(key))groups.set(key,{stage:g.stage,opponent,games:[],wins:0,losses:0,stats:[]});
    const x=groups.get(key);
    x.games.push(g);if(stat)x.stats.push(stat);
  ...
  return <>
    <div className="sectionHead">
      <div><span className="eyebrow">POSTSEASON · {universe.name}</span><h1>{en?"Playoff Hub":"Playoff Hub"}</h1></div>
    ...
    {series.length?series.map((s:any)=><section className="panel seriesPanel" key={s.stage+"-"+s.opponent?.abbreviation}>
      <div className="seriesHeader">
        <div><span className="eyebrow">{s.stage}</span><h2>{career.current_team?.abbreviation} vs {s.opponent?.abbreviation}</h2></div>
        <div className="seriesScore">{s.wins}–{s.losses}</div>
      </div>
      <div className="seriesTeams"><TeamBadge team={career.current_team}/><span>VS</span><TeamBadge team={s.opponent}/></div>
      ...
      <div className="seriesGames">{s.games.map((g:any,i:number)=><Link href={"/game/"+g.id+(g.status==="completed"?"":"#stats")} className="seriesGame" key={g.id}>
        <span>Game {i+1}</span><b>{g.away?.abbreviation} @ {g.home?.abbreviation}</b>
