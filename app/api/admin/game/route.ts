...
export async function POST(req:Request){try{await requireAdmin();const b=await req.json();const client=db();const {data:career}=await client.from("career_profiles").select("*").limit(1).single();if(!career)throw new Error("Career profile not found");const {data:game}=await client.from("games").select("*").eq("id",b.gameId).single();if(!game)return NextResponse.json({error:"Spiel nicht gefunden"},{status:404});
