...
export default async function Page(){
  ...
  const [{data:trophies},{data:records},{data:recaps},{data:legacy},{data:seasons},{data:awards}]=await Promise.all([
  ...
  return <>
    ...
    <div className="sectionHead"><h2>{en?"Career Records":"Karriererekorde"}</h2></div>
    <div className="statGrid">{(records||[]).map((r:any)=><div className="statCard" key={r.id}><span>{r.label}</span><strong>{r.value}</strong></div>)}</div>
