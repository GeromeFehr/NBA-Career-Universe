...
export default function MediaCard({post}:{post:any}){
  return <article className={`mediaCard tone-${String(post.tone||"").toLowerCase()} kind-${String(post.kind||"").toLowerCase()}`}>
    ...
    <footer>{post.author_name||"Staff"} {post.game_id&&<>· <Link href={`/game/${post.game_id}`}>Game ↗</Link></>}</footer>
  </article>
}
