...
function usd(v:any){return "$"+Number(v||0).toFixed(Number(v||0)<0.1?4:2)}
...
export default function AiUsageMeter({language,compact=false}:{language:"de"|"en";compact?:boolean}){
  ...
  if(compact)return <div className="card usageCompact">
    ...
    <small>{en?"this season":"diese Saison"} · {data.season.requests} Requests</small>
  ...
  return <section className="panel">
    ...
    <div className="statGrid">
      <div className="statCard"><span>{en?"Today":"Heute"}</span><strong>{usd(data.today.costUsd)}</strong><small>{data.today.requests} Requests</small></div>
      ...
      <div className="statCard"><span>{en?"Career total":"Karriere gesamt"}</span><strong>{usd(data.total.costUsd)}</strong><small>{data.total.requests} Requests</small></div>
